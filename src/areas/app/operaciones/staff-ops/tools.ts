...
export const tools = {
  ...
  staff_ops_list: {
    description: '[Staff Ops] Listar empleados de una empresa. Si no se pasa company_id usa la empresa por defecto.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        search: { type: 'string', description: 'Buscar por nombre o email' },
        company_id: { type: 'string', description: 'UUID de la empresa (opcional, usa default si no se pasa)' },
      },
    },
    handler: async (args: any) => {
      const params = new URLSearchParams();
      if (args.search) params.set('search', args.search);
      const res = await api.get(`/staff/employees?${params.toString()}`, args.company_id);
  ...
  staff_ops_create: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        company_id: { type: 'string', description: 'UUID de la empresa (opcional)' },
      },
  ...
  staff_ops_get: {
    ...
    handler: async (args: any) => {
      if (!args.employee_id) return err('employee_id es requerido');
      const res = await api.get(`/staff/employees/${args.employee_id}`, args.company_id);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
  ...
  staff_ops_update: {
    ...
    handler: async (args: any) => {
      ...
      const res = await api.put(`/staff/${args.employee_id}`, body, args.company_id);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
  ...
  staff_ops_delete: {
    ...
    handler: async (args: any) => {
      if (!args.employee_id) return err('employee_id es requerido');
      const res = await api.del(`/staff/${args.employee_id}`, args.company_id);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
