...
export const tools = {
  ...
  shipments_ops_record_finding: {
    description: '[Shipments Ops] Registrar hallazgo de conciliacion en una remesa. Tipos: status_mismatch, value_mismatch, zone_mismatch, missing_in_system, missing_in_courier, late_delivery, retained_no_reason, unauthorized_return, flete_discrepancy, payment_not_collected.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        remittance_id: { type: 'string', description: 'UUID de la remesa' },
        shipment_id: { type: 'string', description: 'UUID del envio (opcional)' },
        type: { type: 'string', enum: ['status_mismatch', 'value_mismatch', 'zone_mismatch', 'missing_in_system', 'missing_in_courier', 'late_delivery', 'retained_no_reason', 'unauthorized_return', 'flete_discrepancy', 'payment_not_collected'], description: 'Tipo de hallazgo' },
        severity: { type: 'string', enum: ['info', 'warning', 'critical'], description: 'Severidad (default: warning)' },
        description: { type: 'string', description: 'Descripcion del hallazgo' },
        courier_data: { type: 'object', description: 'Datos reportados por el courier (estado, valor, fecha)' },
        system_data: { type: 'object', description: 'Datos que tiene Novalogic' },
        source: { type: 'string', enum: ['auto_reconciliation', 'manual'], description: 'Origen (default: manual)' },
      },
      required: ['remittance_id', 'type', 'description'],
    },
    handler: async (args: any) => {
      const body: any = {
        type: args.type,
      ...
      if (args.shipment_id) body.shipmentId = args.shipment_id;
      if (args.severity) body.severity = args.severity;
      if (args.courier_data) body.courierData = args.courier_data;
      if (args.system_data) body.systemData = args.system_data;
      if (args.source) body.source = args.source;
      const res = await api.post(`/reconciliation-findings/${args.remittance_id}`, body);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
      return ok({ finding: res.data }, 'Hallazgo registrado');
    },
  },

  shipments_ops_list_findings: {
    description: '[Shipments Ops] Listar hallazgos de conciliacion de una remesa.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        remittance_id: { type: 'string', description: 'UUID de la remesa' },
    ...
    handler: async (args: any) => {
      const res = await api.get(`/reconciliation-findings/${args.remittance_id}`);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
      return ok({ findings: res.data });
    },
  },

  shipments_ops_resolve_finding: {
    description: '[Shipments Ops] Actualizar o resolver un hallazgo de conciliacion.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        remittance_id: { type: 'string', description: 'UUID de la remesa' },
        finding_id: { type: 'string', description: 'UUID del hallazgo' },
        status: { type: 'string', enum: ['open', 'acknowledged', 'disputed', 'resolved'], description: 'Nuevo estado' },
        resolution: { type: 'string', description: 'Descripcion de la resolucion' },
      },
      required: ['remittance_id', 'finding_id'],
    },
    handler: async (args: any) => {
      const body: any = {};
      if (args.status) body.status = args.status;
      if (args.resolution) body.resolution = args.resolution;
      const res = await api.patch(`/reconciliation-findings/${args.remittance_id}/${args.finding_id}`, body);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
      return ok({ finding: res.data }, 'Hallazgo actualizado');
    },
  },

  shipments_ops_findings_summary: {
    description: '[Shipments Ops] Resumen de hallazgos de conciliacion por remesa (conteo por tipo y severidad).',
    inputSchema: {
      type: 'object' as const,
      properties: {
        remittance_id: { type: 'string', description: 'UUID de la remesa' },
    ...
    handler: async (args: any) => {
      const res = await api.get(`/reconciliation-findings/${args.remittance_id}/summary`);
      if (!res.ok) return err(`Error ${res.status}: ${JSON.stringify(res.data)}`);
